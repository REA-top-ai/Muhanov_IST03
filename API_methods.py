...
def _make_request(endpoint: str, api_key: str, params: dict = None) -> dict:
    ...
    if params:
        default_params.update(params) #если есть какие-то параметры, то добавляет их к api
    try:
        response = r.get(url, params=default_params, timeout = 10)
        return response.json() #обрабатывает вводную строку и преобразовывает ее в список
...
def summarize_with_mistral(api_key: str, articles: list[dict]) -> str:
    ...
    articles_text = ""
    for i, article in enumerate(articles[:10], 1):  # ограничим
